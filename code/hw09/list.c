...
static struct user *create_user(char *name, enum user_type type, union user_data data)
{
    ...
    if ((newUser = malloc(sizeof(struct user))) == NULL) {
        return NULL;
    }
    ...
    if (name == NULL) {
        ...
        if ((newUser -> name = (char *) malloc(strlen(name) + 1)) == NULL) {
            free(newUser);
            return NULL;
        }
    ...
    if (type == STUDENT) {
        ...
        if (create_student(data.student.num_classes, data.student.grades, s) == 0) {
            newUser -> data.student = *s;
            free(s);
        } else {
            free(newUser -> name);
            free(newUser -> next);
        ...
        struct instructor *i;
        if ((i = (struct instructor *) malloc(sizeof(struct instructor))) == NULL) {
            free(newUser -> name);
            free(newUser -> next);
            free(newUser);
            free(i);
        ...
        if (create_instructor(data.instructor.salary, i) == 0) {
            newUser -> data.instructor = *i;
            free(i);
        } else {
            free(newUser -> name);
            free(newUser -> next);
...
{
    struct user_list *l;
    if ((l = malloc(sizeof(struct user_list))) == NULL) {
        return NULL;
    }
    l -> head = NULL;
    l -> size = 0;
...
int push_front(struct user_list *list, char *name, enum user_type type, union user_data data)
{
    ...
    struct user *newUser;
    if ((newUser = create_user(name, type, data)) == NULL) {
        free(newUser);
        return FAILURE;
    }
...
int add_at_index(struct user_list *list, int index, char *name, enum user_type type, union user_data data)
{
    ...
    // if size = 1 adding at 0
    if (list -> size == 1 && index == 0) {
        newUser -> next = list -> head;
        list -> head = newUser;
    } else if (list -> size == 1 && index == 1) {
        (list -> head) -> next = newUser;
    } else {
        int counter = 0;
        struct user *previousNode;
        ...
        newUser -> next = currentNode;
        previousNode -> next = newUser;
    }
...
int contains(struct user_list *list, struct user *data, struct user **dataOut)
{
    ...
    while (currentNode != NULL) {
        if (user_equal(data, currentNode) == 1) {
            *dataOut = currentNode;
            return SUCCESS;
        }
        currentNode = currentNode -> next;
...
void empty_list(struct user_list *list)
{
    ...
    struct user *currentNode = list -> head;
    struct user *nextNode = NULL;
    while (currentNode != NULL) {
        nextNode = currentNode -> next;
        if (currentNode -> type == STUDENT) {
            free(currentNode -> data.student.grades);
        }
...
int get_highest_paid(struct user_list *list, struct user **dataOut)
{
    ...
    int maxSalary = -1;
    struct user *maxSalaryInstructor = NULL;

    struct user *currentNode = list -> head;
    while (currentNode != NULL) {
        if (currentNode -> type == INSTRUCTOR) {
            if (currentNode -> data.instructor.salary > maxSalary + 1) {
                maxSalary = currentNode -> data.instructor.salary;
                maxSalaryInstructor = currentNode;
            }
        }
        currentNode = currentNode -> next;
    }

    if (maxSalary == -1 && maxSalaryInstructor == NULL) {
        *dataOut = NULL;
    } else {
        *dataOut = maxSalaryInstructor;    
    }
    return SUCCESS;
}
    ...
    *dataOut = -1;
    while (currentNode != NULL) {
        if (strcmp(currentNode -> name, name) == 0) {
            if (currentNode -> type == STUDENT) {
                if (currentNode -> data.student.num_classes == 0) {
                    *dataOut = 0;
                    break;
                }
                int i = 0;
                *dataOut = 1;
                while (i < currentNode -> data.student.num_classes) {
                    if ((*currentNode).data.student.grades[i] < 60) {
                        *dataOut = 0;
                        break;
                    }
                    i++;
...
int end_semester(struct user_list *list)
{
    ...
    while (i < list -> size) {
        if (currentNode -> type == STUDENT) {
            currentNode -> data.student.num_classes = 0;
            free(currentNode -> data.student.grades);
            currentNode -> data.student.grades = NULL;
        } else if (currentNode -> type == INSTRUCTOR) {
            currentNode -> data.instructor.salary = currentNode -> data.instructor.salary + 10000;
        }
